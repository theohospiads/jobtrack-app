"use client"

import { TopNav } from "@/components/top-nav"
import { useParams, useRouter } from "next/navigation"
import { useState } from "react"

const opportunities: Record<string, any> = {
  "1": {
    id: "1",
    title: "Product Analyst Intern",
    company: "Acme Corp",
    location: "Paris, France",
    salary: "$65,000 - $75,000",
    matchScore: 92,
  },
  "2": {
    id: "2",
    title: "Data Analyst",
    company: "TechStart Inc",
    location: "London, UK",
    salary: "$72,000 - $88,000",
    matchScore: 88,
  },
  "3": {
    id: "3",
    title: "Business Analyst",
    company: "DataFlow",
    location: "New York, USA",
    salary: "$78,000 - $95,000",
    matchScore: 85,
  },
}

export default function ApplyPage() {
  const params = useParams()
  const router = useRouter()
  const id = params.id as string
  const opportunity = opportunities[id] || opportunities["1"]

  const [applicationMessage, setApplicationMessage] = useState(
    "I'm excited to apply for the " + opportunity.title + " position at " + opportunity.company + ". My background in product analytics and data-driven decision making aligns well with your needs. I've led multiple projects that improved user retention by 20%+ and have strong experience with SQL, Python, and data visualization tools."
  )
  const [selectedCV, setSelectedCV] = useState("GeneratedCV_2024.pdf")
  const [applicationState, setApplicationState] = useState<"ready" | "submitted" | "saved">("ready")

  const handleApply = () => {
    setApplicationState("submitted")
  }

  const handleSave = () => {
    setApplicationState("saved")
  }

  return (
    <div style={{ background: "#F8FAFC", minHeight: "100vh" }}>
      <TopNav />

      <main style={{ maxWidth: "900px", margin: "0 auto", padding: "32px 16px" }}>
        {/* Back Button */}
        <button
          onClick={() => router.back()}
          style={{
            display: "flex",
            alignItems: "center",
            gap: 8,
            marginBottom: 24,
            padding: "8px 12px",
            background: "transparent",
            border: "none",
            cursor: "pointer",
            color: "#2563EB",
            fontSize: 14,
            fontWeight: 400,
            transition: "all 0.3s ease",
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.opacity = "0.7"
            e.currentTarget.style.transform = "translateX(-4px)"
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.opacity = "1"
            e.currentTarget.style.transform = "translateX(0)"
          }}
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M10 12L6 8L10 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          Back to Opportunity
        </button>

        {applicationState === "ready" && (
          <>
            {/* Page Title */}
            <h1 style={{ fontSize: 28, fontWeight: 700, color: "#0F172A", margin: "0 0 24px 0" }}>
              Apply now
            </h1>

            {/* Job Context Card */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 20,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 12 }}>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Role
                  </p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>
                    {opportunity.title}
                  </p>
                </div>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Company
                  </p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>
                    {opportunity.company}
                  </p>
                </div>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Location
                  </p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>
                    {opportunity.location}
                  </p>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, borderTop: "1px solid #E5E7EB", paddingTop: 12 }}>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Salary
                  </p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>
                    {opportunity.salary}
                  </p>
                </div>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Match Score
                  </p>
                  <p style={{ fontSize: 14, color: "#2563EB", margin: 0, fontWeight: 600 }}>
                    {opportunity.matchScore}%
                  </p>
                </div>
                <div />
              </div>
            </div>

            {/* Application Message Section */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 24,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                Application Message
              </p>
              <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 16px 0" }}>
                Optimized cover letter tailored to this role
              </p>

              <textarea
                value={applicationMessage}
                onChange={(e) => setApplicationMessage(e.target.value)}
                style={{
                  width: "100%",
                  minHeight: 120,
                  padding: 12,
                  border: "1px solid #E5E7EB",
                  borderRadius: 8,
                  fontSize: 14,
                  fontFamily: "inherit",
                  color: "#0F172A",
                  lineHeight: "1.6",
                }}
              />
            </div>

            {/* CV Submission Section */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 24,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                Selected CV
              </p>
              <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 16px 0" }}>
                This CV will be sent with your application
              </p>

              <div
                style={{
                  background: "#F8FAFC",
                  border: "1px solid #E5E7EB",
                  borderRadius: 8,
                  padding: 16,
                  display: "flex",
                  alignItems: "center",
                  gap: 12,
                }}
              >
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 2H4C3.44772 2 3 2.44772 3 3V17C3 17.5523 3.44772 18 4 18H16C16.5523 18 17 17.5523 17 17V6L12 2Z" stroke="#64748B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
                  <path d="M12 2V6H16" stroke="#64748B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                <div>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: "0", fontWeight: 500 }}>
                    {selectedCV}
                  </p>
                  <p style={{ fontSize: 12, color: "#64748B", margin: "4px 0 0 0" }}>
                    Generated CV
                  </p>
                </div>
              </div>
            </div>

            {/* Action Buttons */}
            <div style={{ display: "flex", gap: 12 }}>
              <button
                onClick={handleSave}
                style={{
                  flex: 1,
                  background: "#FFFFFF",
                  color: "#2563EB",
                  fontSize: 15,
                  fontWeight: 400,
                  padding: "12px 24px",
                  borderRadius: 8,
                  border: "1px solid #E5E7EB",
                  cursor: "pointer",
                  transition: "all 0.2s",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "#F8FAFC"
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "#FFFFFF"
                }}
              >
                Save for Later
              </button>
              <button
                onClick={handleApply}
                style={{
                  flex: 1,
                  background: "#2563EB",
                  color: "#FFFFFF",
                  fontSize: 15,
                  fontWeight: 400,
                  padding: "12px 24px",
                  borderRadius: 8,
                  border: "none",
                  cursor: "pointer",
                  transition: "all 0.2s",
                  boxShadow: "0 4px 12px rgba(37, 99, 235, 0.3)",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "#1E40AF"
                  e.currentTarget.style.boxShadow = "0 6px 16px rgba(37, 99, 235, 0.4)"
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "#2563EB"
                  e.currentTarget.style.boxShadow = "0 4px 12px rgba(37, 99, 235, 0.3)"
                }}
              >
                Apply & Send
              </button>
            </div>
          </>
        )}

        {applicationState === "submitted" && (
          <div
            style={{
              background: "#F0FDF4",
              border: "1px solid #BBEAD5",
              borderRadius: 12,
              padding: 32,
              textAlign: "center",
              marginTop: 32,
            }}
          >
            <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ margin: "0 auto 16px" }}>
              <circle cx="24" cy="24" r="22" fill="#D1FAE5" stroke="#10B981" strokeWidth="2"/>
              <path d="M18 24L22 28L30 20" stroke="#10B981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#0F172A", margin: "0 0 8px 0" }}>
              Application sent successfully
            </h2>
            <p style={{ fontSize: 14, color: "#64748B", margin: "0 0 24px 0" }}>
              Your application has been submitted. You can track its progress in your applications.
            </p>
            <button
              onClick={() => router.push("/actions/1")}
              style={{
                background: "#2563EB",
                color: "#FFFFFF",
                fontSize: 15,
                fontWeight: 400,
                padding: "12px 24px",
                borderRadius: 8,
                border: "none",
                cursor: "pointer",
                transition: "all 0.2s",
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.background = "#1E40AF"
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = "#2563EB"
              }}
            >
              View Application Details
            </button>
          </div>
        )}

        {applicationState === "saved" && (
          <div
            style={{
              background: "#F8FAFC",
              border: "1px solid #E5E7EB",
              borderRadius: 12,
              padding: 32,
              textAlign: "center",
              marginTop: 32,
            }}
          >
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#0F172A", margin: "0 0 8px 0" }}>
              Application saved
            </h2>
            <p style={{ fontSize: 14, color: "#64748B", margin: "0 0 24px 0" }}>
              Your application draft has been saved. You can resume it anytime.
            </p>
            <button
              onClick={() => router.push(`/opportunities/${id}`)}
              style={{
                background: "#2563EB",
                color: "#FFFFFF",
                fontSize: 15,
                fontWeight: 400,
                padding: "12px 24px",
                borderRadius: 8,
                border: "none",
                cursor: "pointer",
                transition: "all 0.2s",
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.background = "#1E40AF"
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = "#2563EB"
              }}
            >
              Back to Opportunity
            </button>
          </div>
        )}
      </main>
    </div>
  )
}

export default function ApplyPage() {
  const params = useParams()
  const router = useRouter()
  const id = params.id as string
  const opportunity = opportunities[id] || opportunities["1"]

  const [applicationMessage, setApplicationMessage] = useState(
    "I'm excited to apply for the " + opportunity.title + " position at " + opportunity.company + ". My background in product analytics and data-driven decision making aligns well with your needs. I've led multiple projects that improved user retention by 20%+ and have strong experience with SQL, Python, and data visualization tools."
  )
  const [selectedCV, setSelectedCV] = useState("GeneratedCV_2024.pdf")
  const [applicationState, setApplicationState] = useState<"ready" | "submitted" | "saved">("ready")

  const handleApply = () => {
    setApplicationState("submitted")
  }

  const handleSave = () => {
    setApplicationState("saved")
  }

  return (
    <div style={{ background: "#F8FAFC", minHeight: "100vh" }}>
      <TopNav />

      <main style={{ maxWidth: "900px", margin: "0 auto", padding: "32px 16px" }}>
        {/* Back Button */}
        <button
          onClick={() => router.back()}
          style={{
            display: "flex",
            alignItems: "center",
            gap: 8,
            marginBottom: 24,
            padding: "8px 12px",
            background: "transparent",
            border: "none",
            cursor: "pointer",
            color: "#2563EB",
            fontSize: 14,
            fontWeight: 400,
            transition: "all 0.3s ease",
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.opacity = "0.7"
            e.currentTarget.style.transform = "translateX(-4px)"
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.opacity = "1"
            e.currentTarget.style.transform = "translateX(0)"
          }}
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M10 12L6 8L10 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          Back to Opportunity
        </button>

        {applicationState === "ready" && (
          <>
            {/* Page Title */}
            <h1 style={{ fontSize: 28, fontWeight: 700, color: "#0F172A", margin: "0 0 24px 0" }}>
              Apply now
            </h1>

            {/* Opportunity Context Card */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 20,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 16, marginBottom: 12 }}>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Role
                  </p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>
                    {opportunity.title}
                  </p>
                </div>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Company
                  </p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>
                    {opportunity.company}
                  </p>
                </div>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Location
                  </p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>
                    {opportunity.location}
                  </p>
                </div>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                    Salary
                  </p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>
                    {opportunity.salary}
                  </p>
                </div>
              </div>
              <div style={{ borderTop: "1px solid #E5E7EB", paddingTop: 12 }}>
                <p style={{ fontSize: 12, color: "#64748B", margin: 0 }}>
                  Match Score: <span style={{ fontWeight: 600, color: "#2563EB" }}>{opportunity.matchScore}%</span>
                </p>
              </div>
            </div>

            {/* Application Message Section */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 24,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                Application message
              </p>
              <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 16px 0" }}>
                Personalized message optimized based on your profile and the role
              </p>

              <textarea
                value={applicationMessage}
                onChange={(e) => setApplicationMessage(e.target.value)}
                style={{
                  width: "100%",
                  minHeight: 120,
                  padding: 12,
                  border: "1px solid #E5E7EB",
                  borderRadius: 8,
                  fontSize: 14,
                  fontFamily: "inherit",
                  color: "#0F172A",
                  lineHeight: "1.6",
                  marginBottom: 16,
                }}
              />

              {/* Profile Strength Indicator */}
              <div
                style={{
                  background: "#F0F9FF",
                  border: "1px solid #BFDBFE",
                  borderRadius: 8,
                  padding: 12,
                }}
              >
                <p style={{ fontSize: 12, fontWeight: 600, color: "#2563EB", margin: "0 0 4px 0" }}>
                  Your profile strength for this role
                </p>
                <p style={{ fontSize: 12, color: "#0F172A", margin: 0 }}>
                  {opportunity.matchScore}% match — excellent fit
                </p>
              </div>
            </div>

            {/* Selected CV Section */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 24,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 4px 0", textTransform: "uppercase" }}>
                CV
              </p>
              <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 16px 0" }}>
                Generated and optimized for this role
              </p>
              <div
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: 12,
                  padding: 12,
                  background: "#F8FAFC",
                  border: "1px solid #E5E7EB",
                  borderRadius: 8,
                }}
              >
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M10 1C5.03 1 1 5.03 1 10C1 14.97 5.03 19 10 19C14.97 19 19 14.97 19 10C19 5.03 14.97 1 10 1ZM14 9L9 14L6 11" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                <span style={{ fontSize: 14, color: "#0F172A", fontWeight: 500 }}>{selectedCV}</span>
              </div>
            </div>

            {/* System Notes */}
            <div style={{ marginBottom: 24 }}>
              <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 4px 0" }}>
                • Your application will be submitted with the generated CV
              </p>
              <p style={{ fontSize: 12, color: "#64748B", margin: 0 }}>
                • You can edit your message before submitting
              </p>
            </div>

            {/* Action Buttons */}
            <div style={{ display: "flex", gap: 12 }}>
              <button
                onClick={handleSave}
                style={{
                  flex: 1,
                  background: "#FFFFFF",
                  color: "#2563EB",
                  fontSize: 15,
                  fontWeight: 400,
                  padding: "12px 24px",
                  borderRadius: 8,
                  border: "1px solid #E5E7EB",
                  cursor: "pointer",
                  transition: "all 0.2s",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "#F8FAFC"
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "#FFFFFF"
                }}
              >
                Save for Later
              </button>
              <button
                onClick={handleApply}
                style={{
                  flex: 1,
                  background: "#2563EB",
                  color: "#FFFFFF",
                  fontSize: 15,
                  fontWeight: 400,
                  padding: "12px 24px",
                  borderRadius: 8,
                  border: "none",
                  cursor: "pointer",
                  transition: "all 0.2s",
                  boxShadow: "0 4px 12px rgba(37, 99, 235, 0.3)",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "#1E40AF"
                  e.currentTarget.style.boxShadow = "0 6px 16px rgba(37, 99, 235, 0.4)"
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "#2563EB"
                  e.currentTarget.style.boxShadow = "0 4px 12px rgba(37, 99, 235, 0.3)"
                }}
              >
                Apply & Send
              </button>
            </div>
          </>
        )}

        {applicationState === "submitted" && (
          <div
            style={{
              background: "#F0FDF4",
              border: "1px solid #BBEAD5",
              borderRadius: 12,
              padding: 32,
              textAlign: "center",
              marginTop: 32,
            }}
          >
            <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ margin: "0 auto 16px" }}>
              <circle cx="24" cy="24" r="22" fill="#D1FAE5" stroke="#10B981" strokeWidth="2"/>
              <path d="M18 24L22 28L30 20" stroke="#10B981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#0F172A", margin: "0 0 8px 0" }}>
              Application submitted successfully
            </h2>
            <p style={{ fontSize: 14, color: "#64748B", margin: "0 0 24px 0" }}>
              Your application for {opportunity.title} at {opportunity.company} has been sent. Track your progress from your applications dashboard.
            </p>
            <button
              onClick={() => router.push(`/opportunities/${id}`)}
              style={{
                background: "#2563EB",
                color: "#FFFFFF",
                fontSize: 15,
                fontWeight: 400,
                padding: "12px 24px",
                borderRadius: 8,
                border: "none",
                cursor: "pointer",
                transition: "all 0.2s",
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.background = "#1E40AF"
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = "#2563EB"
              }}
            >
              Back to Opportunity
            </button>
          </div>
        )}

        {applicationState === "saved" && (
          <div
            style={{
              background: "#F8FAFC",
              border: "1px solid #E5E7EB",
              borderRadius: 12,
              padding: 32,
              textAlign: "center",
              marginTop: 32,
            }}
          >
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#0F172A", margin: "0 0 8px 0" }}>
              Application saved
            </h2>
            <p style={{ fontSize: 14, color: "#64748B", margin: "0 0 24px 0" }}>
              Your draft has been saved. You can continue editing and submit whenever you're ready.
            </p>
            <button
              onClick={() => router.push(`/opportunities/${id}`)}
              style={{
                background: "#2563EB",
                color: "#FFFFFF",
                fontSize: 15,
                fontWeight: 400,
                padding: "12px 24px",
                borderRadius: 8,
                border: "none",
                cursor: "pointer",
                transition: "all 0.2s",
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.background = "#1E40AF"
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = "#2563EB"
              }}
            >
              Back to Opportunity
            </button>
          </div>
        )}
      </main>
    </div>
  )
}
        <button
          onClick={() => router.back()}
          style={{
            display: "flex",
            alignItems: "center",
            gap: 8,
            marginBottom: 24,
            padding: "8px 12px",
            background: "transparent",
            border: "none",
            cursor: "pointer",
            color: "#2563EB",
            fontSize: 14,
            fontWeight: 400,
            transition: "all 0.3s ease",
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.opacity = "0.7"
            e.currentTarget.style.transform = "translateX(-4px)"
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.opacity = "1"
            e.currentTarget.style.transform = "translateX(0)"
          }}
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M10 12L6 8L10 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          Back
        </button>

        {applicationState === "submitted" ? (
          // Success State
          <div
            style={{
              background: "#FFFFFF",
              border: "1px solid #E5E7EB",
              borderRadius: 12,
              padding: 32,
              textAlign: "center",
              boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
            }}
          >
            <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ margin: "0 auto 16px" }}>
              <circle cx="24" cy="24" r="24" fill="#DCFCE7" />
              <path d="M20 24L22 26L28 20" stroke="#16A34A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <h2 style={{ fontSize: 24, fontWeight: 700, color: "#0F172A", margin: "0 0 8px 0" }}>
              Application Submitted!
            </h2>
            <p style={{ fontSize: 14, color: "#64748B", margin: "0 0 24px 0" }}>
              Your application has been sent. We'll notify you when the recruiter reviews it.
            </p>
            <button
              onClick={() => router.push(`/actions/${id}`)}
              style={{
                background: "#2563EB",
                color: "#FFFFFF",
                fontSize: 15,
                fontWeight: 400,
                padding: "12px 24px",
                borderRadius: 8,
                border: "none",
                cursor: "pointer",
                transition: "all 0.2s",
                boxShadow: "0 4px 12px rgba(37, 99, 235, 0.3)",
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.background = "#1E40AF"
                e.currentTarget.style.boxShadow = "0 6px 16px rgba(37, 99, 235, 0.4)"
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = "#2563EB"
                e.currentTarget.style.boxShadow = "0 4px 12px rgba(37, 99, 235, 0.3)"
              }}
            >
              View Application Details
            </button>
          </div>
        ) : applicationState === "saved" ? (
          // Saved State
          <div
            style={{
              background: "#FFFFFF",
              border: "1px solid #E5E7EB",
              borderRadius: 12,
              padding: 32,
              textAlign: "center",
              boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
            }}
          >
            <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ margin: "0 auto 16px" }}>
              <rect width="48" height="48" fill="#FEF3C7" rx="24" />
              <path d="M24 16V28M24 28L20 24M24 28L28 24" stroke="#FBBF24" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <h2 style={{ fontSize: 24, fontWeight: 700, color: "#0F172A", margin: "0 0 8px 0" }}>
              Draft Saved
            </h2>
            <p style={{ fontSize: 14, color: "#64748B", margin: "0 0 24px 0" }}>
              Your application draft has been saved. You can resume anytime.
            </p>
            <button
              onClick={() => setApplicationState("ready")}
              style={{
                background: "#2563EB",
                color: "#FFFFFF",
                fontSize: 15,
                fontWeight: 400,
                padding: "12px 24px",
                borderRadius: 8,
                border: "none",
                cursor: "pointer",
              }}
            >
              Resume Application
            </button>
          </div>
        ) : (
          <>
            {/* Job Recap */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 24,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <h2 style={{ fontSize: 18, fontWeight: 700, color: "#0F172A", margin: "0 0 16px 0" }}>
                Product Analyst Intern
              </h2>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                <div>
                  <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 4px 0", fontWeight: 600 }}>Company</p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>Acme Corp</p>
                </div>
                <div>
                  <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 4px 0", fontWeight: 600 }}>Location</p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>Paris, France</p>
                </div>
                <div>
                  <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 4px 0", fontWeight: 600 }}>Salary</p>
                  <p style={{ fontSize: 14, color: "#0F172A", margin: 0 }}>$65,000 - $75,000</p>
                </div>
                <div>
                  <p style={{ fontSize: 12, color: "#64748B", margin: "0 0 4px 0", fontWeight: 600 }}>Match</p>
                  <p style={{ fontSize: 14, color: "#2563EB", margin: 0, fontWeight: 600 }}>92% match</p>
                </div>
              </div>
            </div>

            {/* Generated Application Message */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 24,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 12px 0", textTransform: "uppercase" }}>
                Your Application Message
              </p>
              <textarea
                value={applicationMessage}
                onChange={(e) => setApplicationMessage(e.target.value)}
                style={{
                  width: "100%",
                  minHeight: 120,
                  padding: 12,
                  border: "1px solid #E5E7EB",
                  borderRadius: 8,
                  fontSize: 14,
                  color: "#0F172A",
                  fontFamily: "inherit",
                  resize: "vertical",
                }}
              />
              <p style={{ fontSize: 12, color: "#64748B", margin: "8px 0 0 0" }}>
                Tip: This message has been optimized for your profile and target role.
              </p>
            </div>

            {/* Selected CV */}
            <div
              style={{
                background: "#FFFFFF",
                border: "1px solid #E5E7EB",
                borderRadius: 12,
                padding: 24,
                marginBottom: 24,
                boxShadow: "0 4px 12px rgba(0, 0, 0, 0.08), 0 1px 3px rgba(0, 0, 0, 0.04)",
              }}
            >
              <p style={{ fontSize: 12, fontWeight: 600, color: "#64748B", margin: "0 0 12px 0", textTransform: "uppercase" }}>
                CV Being Submitted
              </p>
              <div
                style={{
                  background: "#F8FAFC",
                  border: "1px solid #E5E7EB",
                  borderRadius: 8,
                  padding: 12,
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "space-between",
                }}
              >
                <div>
                  <p style={{ fontSize: 14, fontWeight: 600, color: "#0F172A", margin: 0 }}>
                    {selectedCV}
                  </p>
                  <p style={{ fontSize: 12, color: "#64748B", margin: "4px 0 0 0" }}>
                    Generated • Optimized for this role
                  </p>
                </div>
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M8 14L4 10M4 10L2 12M4 10H16C17.1046 10H18V2C18 0.895431 17.1046 0 16 0H2C0.895431 0 0 0.895431 0 2V12" stroke="#2563EB" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
            </div>

            {/* Action Buttons */}
            <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
              <button
                onClick={handleSave}
                style={{
                  flex: 1,
                  background: "#FFFFFF",
                  color: "#2563EB",
                  fontSize: 15,
                  fontWeight: 400,
                  padding: "12px 24px",
                  borderRadius: 8,
                  border: "1px solid #E5E7EB",
                  cursor: "pointer",
                  transition: "all 0.2s",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "#F8FAFC"
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "#FFFFFF"
                }}
              >
                Save for Later
              </button>
              <button
                onClick={handleApply}
                style={{
                  flex: 1,
                  background: "#2563EB",
                  color: "#FFFFFF",
                  fontSize: 15,
                  fontWeight: 400,
                  padding: "12px 24px",
                  borderRadius: 8,
                  border: "none",
                  cursor: "pointer",
                  transition: "all 0.2s",
                  boxShadow: "0 4px 12px rgba(37, 99, 235, 0.3)",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "#1E40AF"
                  e.currentTarget.style.boxShadow = "0 6px 16px rgba(37, 99, 235, 0.4)"
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "#2563EB"
                  e.currentTarget.style.boxShadow = "0 4px 12px rgba(37, 99, 235, 0.3)"
                }}
              >
                Apply & Send
              </button>
            </div>
          </>
        )}
      </main>
    </div>
  )
}
